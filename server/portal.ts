...
import { getCompany, getDetail, chargeInvoice, getInvoice, settleIntent } from './invoices.ts';
...
export function createLoginLink(db: DB, clientId: number, validMs = INVITE_DAYS * 86_400_000): string {
  const token = newToken();
  db.prepare('INSERT INTO portal_tokens (client_id, token_hash, expires_at) VALUES (?, ?, ?)').run(clientId, hash(token), inFuture(validMs));
  return `/portal/login?token=${token}`;
}
...
export function createPortalApi(db: DB, pay: Payments): Router {
  ...
  api.post(
    ...
    h((req) => {
      ...
      if (job.status !== 'quoted') throw new HttpError(409, 'This estimate is no longer open for approval');
      db.prepare('UPDATE jobs SET approved_at = ? WHERE id = ?').run(now(), job.id);
      logActivity(db, 'portal', `✅ ${client.name} approved estimate ${job.number} in the portal`, { jobId: job.id });
      return publicJob(db, job.id);
    }),
  );
