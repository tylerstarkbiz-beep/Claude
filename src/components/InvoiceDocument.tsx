...
const cellInput = 'w-full rounded bg-transparent px-1 py-0.5 outline-none hover:bg-slate-50 focus:bg-indigo-50 print:hover:bg-transparent';
...
export function InvoiceDocument({
  ...
  invoice: Pick<InvoiceDetail, 'number' | 'issueDate' | 'dueDate' | 'items' | 'subtotal' | 'tax' | 'taxRate' | 'total' | 'amountPaid' | 'balance' | 'notes' | 'client' | 'status'> & {
  ...
  return (
    <div className="print-area card p-6 sm:p-10">
      <div className="mb-8 flex flex-wrap justify-between gap-6">
        ...
        <div className="text-right">
          <div className="text-3xl font-light tracking-wide text-slate-400">INVOICE</div>
          <div className="mt-1 font-mono text-sm">{invoice.number}</div>
          {invoice.status === 'paid' && <div className="mt-2 inline-block rounded border-2 border-emerald-600 px-2 text-sm font-bold text-emerald-600">PAID</div>}
      ...
      <div className="mb-8 grid gap-6 text-sm sm:grid-cols-3">
        ...
        <div className="sm:text-right">
          ...
          <div>{shortDate(invoice.issueDate)}, {invoice.issueDate.slice(0, 4)}</div>
          <div className="label mt-2">Due</div>
          <div className="font-medium">{shortDate(invoice.dueDate)}, {invoice.dueDate.slice(0, 4)}</div>
        </div>
      </div>

      <table className="w-full text-sm">
        <thead className="border-b-2 border-slate-800 text-left text-xs uppercase tracking-wide text-slate-500">
          <tr>
          ...
          {invoice.items.map((it) => (
            <tr key={it.id} className="group border-b border-slate-100">
              <td className="py-2 pr-2">
                {editable ? (
                  <input className={cellInput} defaultValue={it.description} onBlur={(e) => e.target.value !== it.description && onEditItem!(it, { description: e.target.value })} />
                ) : (
                  it.description
                )}
              </td>
              <td className="py-2 text-right">
                {editable ? (
                  <input type="number" step="any" className={cellInput + ' text-right'} defaultValue={it.quantity} onBlur={(e) => Number(e.target.value) !== it.quantity && onEditItem!(it, { quantity: Number(e.target.value) })} />
                ) : (
                  it.quantity
                )}
              </td>
              <td className="py-2 text-right">
                {editable ? (
                  <input type="number" step="any" className={cellInput + ' text-right'} defaultValue={it.unitPrice} onBlur={(e) => Number(e.target.value) !== it.unitPrice && onEditItem!(it, { unitPrice: Number(e.target.value) })} />
                ) : (
                  money(it.unitPrice)
                )}
              </td>
              ...
              {editable && (
                <td className="no-print text-right">
                  <button className="text-slate-300 hover:text-rose-600 sm:invisible sm:group-hover:visible" onClick={() => onDeleteItem?.(it)} aria-label="Remove line">
                    <Trash2 size={14} />
                  </button>
                </td>
              )}
